/*
  Lock down admin-only data and CMS mutations.

  Earlier CMS migrations allowed the browser anon key to read and mutate admin
  tables so the temporary admin bypass could work. The anon key is public, so
  RLS must enforce the same admin identity as the login page.
*/
