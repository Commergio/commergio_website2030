/*
  Lock down CMS/admin RLS policies that previously allowed anonymous reads and
  mutations with the public Supabase anon key.

  Public site behavior preserved:
    - Published/public content can be read by anon/authenticated clients.
    - Contact messages and project leads can be inserted by public forms.
    - Public storage objects remain readable.

  Admin-only behavior:
    - Private tables and all CMS mutations require the authenticated admin email.
*/
