/*
  Lock down CMS/admin RLS policies that previously trusted the public anon key.

  Public visitors may read public CMS content and submit forms. Private data,
  CMS mutations, and storage writes require a Supabase session for the admin
  email because the anon key is embedded in the browser bundle.
*/
...
CREATE POLICY "cms_storage_select"
  ...
  USING (bucket_id IN (
    ...
    bucket_id IN ('partnership-videos', 'partnership-thumbnails')
